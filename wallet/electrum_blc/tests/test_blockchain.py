...
from electrum_blc import constants, blockchain
from electrum_blc.simple_config import SimpleConfig
from electrum_blc.blockchain import Blockchain, deserialize_header, hash_header
from electrum_blc.util import bh2u, bfh, make_dir
...
class TestBlockchain(ElectrumTestCase):
    ...
    def test_deserialize_full_auxpow_header_accepts_variable_length(self):
        ...
        self.assertEqual(
            blockchain.hash_raw_header(bh2u(raw_header)),
            blockchain.hash_raw_header(bh2u(raw_header[:blockchain.HEADER_SIZE])),
        )
...
class TestVerifyHeader(ElectrumTestCase):
    ...
    # Data for Blakecoin block header #100.
