...
from electrum_blc import bitcoin, constants
from electrum_blc import lnutil
from electrum_blc.transaction import tx_from_any, PartialTxOutput
from electrum_blc.util import (parse_URI, InvalidBitcoinURI, TxMinedInfo, maybe_extract_lightning_payment_identifier,
                               InvoiceError, format_time, parse_max_spend, BITCOIN_BIP21_URI_SCHEME)
from electrum_blc.lnaddr import lndecode, LnInvoiceException
from electrum_blc.lnurl import decode_lnurl, request_lnurl, callback_lnurl, LNURLError, LNURL6Data
...
from electrum_blc.gui.kivy import KIVY_GUI_PATH
from electrum_blc.gui.kivy.i18n import _

if TYPE_CHECKING:
    from electrum_blc.gui.kivy.main_window import ElectrumWindow
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Blakecoin address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Blakecoin Address') + ':\n' + address)
                        return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_blc.util import create_bip21_uri
        amount = self.app.get_amount(self.amount)
