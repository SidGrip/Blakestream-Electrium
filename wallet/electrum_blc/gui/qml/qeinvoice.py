...
from electrum_blc.invoices import (PR_UNPAID, PR_EXPIRED, PR_UNKNOWN, PR_PAID, PR_INFLIGHT,
                                   PR_FAILED, PR_ROUTING, PR_UNCONFIRMED)
from electrum_blc.lnaddr import LnInvoiceException
from electrum_blc.logging import get_logger
from electrum_blc.transaction import PartialTxOutput
from electrum_blc.util import (parse_URI, InvalidBitcoinURI, InvoiceError,
                               maybe_extract_lightning_payment_identifier)
from electrum_blc.lnurl import decode_lnurl, request_lnurl, callback_lnurl
...
class QEUserEnteredPayment(QEInvoice):
    ...
    def validate(self):
        ...
        if not bitcoin.is_address(self._recipient):
            self.validationError.emit('recipient', _('Invalid Blakecoin address'))
            return
