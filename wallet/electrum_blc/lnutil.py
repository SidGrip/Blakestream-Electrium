...
# Legacy BOLT funding without the large-channel feature bit is capped below
# 2^24 sat. Keep that protocol ceiling separate from the wallet-side max so
# open_channel can distinguish "legacy peer limit" from "our local wallet
# policy limit".
LN_MAX_FUNDING_SAT_LEGACY = pow(2, 24) - 1
LN_MAX_FUNDING_SAT = bitcoin.TOTAL_COIN_SUPPLY_LIMIT_IN_BTC * bitcoin.COIN
DUST_LIMIT_MAX = 1000
...
class ChannelConfig(StoredObject):
    ...
    def validate_params(
            self, *,
            funding_sat: int,
            is_large_channel: bool = False,
            max_funding_sat: int = LN_MAX_FUNDING_SAT,
    ) -> None:
        ...
        if not is_large_channel and funding_sat > LN_MAX_FUNDING_SAT_LEGACY:
            raise Exception(f"funding_sat too high: {funding_sat} sat > {LN_MAX_FUNDING_SAT_LEGACY} (legacy limit)")
        if funding_sat > max_funding_sat:
            raise Exception(f"funding_sat too high: {funding_sat} sat > {max_funding_sat} (wallet limit)")
    ...
    def cross_validate_params(
        ...
        local_config.validate_params(
            ...
            is_large_channel=is_large_channel,
            max_funding_sat=max_funding_sat,
        )
