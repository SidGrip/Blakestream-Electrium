...
class Peer(Logger):
    ...
    async def process_message(self, message):
        ...
        if message_type in self.ORDERED_MESSAGES:
            ...
            if asyncio.iscoroutinefunction(f):
                ...
                if message_type == 'channel_reestablish':
    ...
    def supports_large_channel(self):
        return self.features.supports(LnFeatures.OPTION_SUPPORT_LARGE_CHANNEL_OPT)
    ...
    def make_local_config(self, funding_sat: int, push_msat: int, initiator: HTLCOwner, channel_type: ChannelType) -> LocalConfig:
        ...
        local_config.validate_params(
            ...
            is_large_channel=self.supports_large_channel(),
        )
        return local_config
    ...
    async def channel_establishment_flow(
        ...
        if funding_sat > lnutil.LN_MAX_FUNDING_SAT_LEGACY and not self.supports_large_channel():
            raise Exception(_('Remote peer does not support large channels.'))
    ...
    async def on_channel_reestablish(self, chan, msg):
        ...
        # Block processing of further incoming messages until we finished our part of
        # channel reestablishment. This preserves replay ordering for byte-for-byte
        # resend of local updates and commitment_signed.
        await self._chan_reest_finished[chan.channel_id].wait()

    def _send_channel_reestablish(self, chan: Channel) -> None:
    ...
    async def reestablish_channel(self, chan: Channel):
        ...
        if chan.peer_state != PeerState.DISCONNECTED:
            ...
            return
        chan_reest_finished = self._chan_reest_finished[chan_id]
        chan_reest_finished.clear()
        chan.peer_state = PeerState.REESTABLISHING
        util.trigger_callback('channel', self.lnworker.wallet, chan)
        try:
            oldest_unrevoked_local_ctn = chan.get_oldest_unrevoked_ctn(LOCAL)
            next_local_ctn = chan.get_next_ctn(LOCAL)
            # BOLT-02: "A node [...] upon disconnection [...] MUST reverse any uncommitted
            # updates sent by the other side"
            ...
            def replay_updates_and_commitsig():
                ...
                self.logger.info(
                    f'channel_reestablish ({chan.get_id_for_log()}): replayed '
                    f'{len(replayed_msgs)} unacked messages. '
    ...
    def send_announcement_signatures(self, chan: Channel):
        ...
        msg_hash = blakecoin_lntx_sighash(preimage)
