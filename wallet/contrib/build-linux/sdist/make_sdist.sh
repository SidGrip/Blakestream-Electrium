...
(
    # Blakecoin currently ships without the upstream locale submodule.
    # Keep an empty locale tree so the sdist layout stays consistent.
    rm -rf "$LOCALE"
    mkdir -p "$LOCALE"
)
...
if ([ "$OMIT_UNCLEAN_FILES" = 1 ]); then
    ...
    rm -f "$PROJECT_ROOT/electrum_blc/paymentrequest_pb2.py"
fi
...
os.rename(f"dist/_sourceonly/Electrum-BLC-{VER}.tar.gz", f"dist/Electrum-BLC-sourceonly-{VER}.tar.gz")
EOF
        rmdir "$PY_DISTDIR"
    fi
)
