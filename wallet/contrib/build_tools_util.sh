...
function touch_reproducible_tree() {
    local root="${1:-.}"
    find "$root" \
        \( -path "$root/.git" \
        -o -path "$root/build" \
        ...
        -o -path "$root/contrib/.venv_make_packages" \
        -o -path "$root/contrib/build-linux/appimage/build" \
        -o -path "$root/contrib/build-linux/appimage/.cache" \
        -o -path "$root/contrib/build-linux/appimage/dist" \
        -o -path "$root/contrib/build-macos/build" \
        -o -path "$root/contrib/build-macos/.cache" \
        -o -path "$root/contrib/build-macos/dist" \
        -o -path "$root/contrib/build-wine/build" \
        -o -path "$root/contrib/build-wine/.cache" \) -prune \
        -o -exec touch -h -d '2000-11-11T11:11:11+00:00' {} +
}

function electrum_package_version() {
    local project_root="${1:-.}"
    local version_file="${project_root}/electrum_blc/version.py"
    local version=""
    if [ -f "$version_file" ]; then
        version="$(sed -n "s/^ELECTRUM_VERSION = ['\"]\\([^'\"]*\\)['\"].*/\\1/p" "$version_file" | head -n 1)"
    fi
    if [ -n "$version" ]; then
        printf '%s\n' "$version"
        return 0
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        fail "Could not determine Electrum version from '$version_file' and python3 is unavailable."
    fi
    python3 - <<PY
ns = {}
with open(r"${version_file}", "r", encoding="utf-8") as fh:
    exec(fh.read(), ns)
