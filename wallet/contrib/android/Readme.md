...
```
$ adb -d install -r dist/Electrum-BLC-*-arm64-v8a-debug.apk
$ adb shell monkey -p org.electrum_blc.electrum_blc 1
```
...
```
adb logcat | grep -F "`adb shell ps | grep org.electrum_blc.electrum_blc | cut -c14-19`"
```
...
Run electrum with the `-g` switch: `electrum-blc -g kivy`
...
```
$ adb shell
$ run-as org.electrum_blc.electrum_blc ls /data/data/org.electrum_blc.electrum_blc/files/data
$ run-as org.electrum_blc.electrum_blc cp /data/data/org.electrum_blc.electrum_blc/files/data/wallets/my_wallet /sdcard/some_path/my_wallet
```
