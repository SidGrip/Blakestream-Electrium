...
pushd $WINEPREFIX/drive_c/electrum-blc

git config --global --add safe.directory /opt/wine64/drive_c/electrum-blc
VERSION="$(electrum_package_version "$WINEPREFIX/drive_c/electrum-blc")"
info "Last commit: $VERSION"

# Blakecoin: skip locale submodule (no translations yet)
LOCALE="$WINEPREFIX/drive_c/electrum-blc/electrum_blc/locale/"
mkdir -p "$LOCALE"
...
cd dist
mv electrum-blc-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
