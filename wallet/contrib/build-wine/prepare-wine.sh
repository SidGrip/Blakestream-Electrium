...
# Blakecoin: Blake-256 PoW hash extension
if [ -f "$DLL_TARGET_DIR/_blake256.pyd" ]; then
    cp "$DLL_TARGET_DIR/_blake256.pyd" $WINEPREFIX/drive_c/tmp/ || fail "Could not copy blake256 to its destination"
fi
...
(
    ...
    cd "$WINEPREFIX/drive_c/electrum-blc"
