...
export WINE_PIP_CACHE_DIR="c:/electrum-blc/contrib/build-wine/.cache/$WIN_ARCH/wine_pip_cache"
...
# Build Blake-256 as a standalone DLL (no Python.h dependency)
# Loaded via ctypes at runtime — avoids needing Windows Python dev headers
info "Building Blake-256 DLL for Windows..."
(
    cd "$PROJECT_ROOT/blake256"
    cat > blake256_dll.c << 'DLLEOF'
#include "sph_blake.h"
#ifdef _WIN32
...
DLLEOF
    ${GCC_TRIPLET_HOST}-gcc -shared -O2 -I. \
        -o "$DLL_TARGET_DIR/blake256.dll" \
        blake256_dll.c blake.c \
        || fail "Could not build Blake-256 DLL for Windows"
)
cp "$DLL_TARGET_DIR/blake256.dll" $WINEPREFIX/drive_c/tmp/ || fail "Could not copy blake256 DLL"
