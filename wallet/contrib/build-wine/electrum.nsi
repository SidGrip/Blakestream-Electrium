...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-BLC"
  !define PRODUCT_WEB_SITE "https://electrum-blc.org/"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-blc-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\electrum_blc\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-blc\*.*"
  File "..\..\electrum_blc\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-blc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-blc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-blc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-blc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-blc-${PRODUCT_VERSION}.exe" 0


  ;Links blakecoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\blakecoin" "" "URL:blakecoin Protocol"
  WriteRegStr HKCU "Software\Classes\blakecoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\blakecoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\blakecoin\shell\open\command" "" "$\"$INSTDIR\electrum-blc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\blakecoin"
