...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/electrum-blc-macos
    cd /tmp/electrum-blc-macos
...
rm -rf /tmp/electrum-blc-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-blc-macos/image/
cp -r $1 /tmp/electrum-blc-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-BLC_uncompressed.dmg \
    /tmp/electrum-blc-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-BLC_uncompressed.dmg electrum-blc-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-BLC_uncompressed.dmg

echo "Done."
sha256sum electrum-blc-$VERSION.dmg
