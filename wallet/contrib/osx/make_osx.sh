...
GIT_REPO=https://github.com/pooler/electrum-blc
...
(
    ...
    rm -rf "$LOCALE"
    "$CONTRIB/build_locale.sh" "$CONTRIB/deterministic-build/electrum-blc-locale/locale/" "$LOCALE"
...
cp -f "$DLL_TARGET_DIR/libsecp256k1.0.dylib" "$PROJECT_ROOT/electrum_blc/" || fail "Could not copy libsecp256k1 dylib"
...
cp -f "$DLL_TARGET_DIR/libzbar.0.dylib" "$PROJECT_ROOT/electrum_blc/" || fail "Could not copy ZBar dylib"
...
cp -f "$DLL_TARGET_DIR/libusb-1.0.dylib" "$PROJECT_ROOT/electrum_blc/" || fail "Could not copy libusb dylib"
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-blc-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-blc-${VERSION}.dmg"
