...
# Set this only when serving legacy wallet clients that still require the
# classic 80-byte header view from ElectrumX on AuxPoW chains.
AUXPOW_TRUNCATE_HEADERS = os.environ.get('BLAKESTREAM_TRUNCATE_AUXPOW_HEADERS', '').lower() in {
    '1', 'true', 'yes', 'on',
}
...
class AuxPoWElectrumX(ElectrumX):
    def maybe_truncate_auxpow(self, header_hex, start_height):
        if not AUXPOW_TRUNCATE_HEADERS:
            # Default path: modern Electrium clients are expected to consume
            # the full variable-length AuxPoW header and normalize it locally.
            return header_hex
        # Compatibility mode for older clients: expose only the canonical
        # 80-byte header prefix even though the backing chain stores full
        # AuxPoW headers.
        return self.truncate_auxpow(header_hex, start_height)

    async def subscribe_headers_result(self):
        result = dict(await super().subscribe_headers_result())
        result['hex'] = self.maybe_truncate_auxpow(result['hex'], result['height'])
        return result

    async def block_header(self, height, cp_height=0):
        result = await super().block_header(height, cp_height)
        if cp_height == 0:
            return self.maybe_truncate_auxpow(result, height)
        result['header'] = self.maybe_truncate_auxpow(result['header'], height)
        return result

    async def block_headers(self, start_height, count, cp_height=0):
        result = await super().block_headers(start_height, count, cp_height)
        result['hex'] = self.maybe_truncate_auxpow(result['hex'], start_height)
        return result
